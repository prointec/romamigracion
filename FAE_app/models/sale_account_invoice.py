...
class AccountMoveReversal(models.TransientModel):
    ...
    def _prepare_default_reversal(self, move):
        ...
        if move.type == 'out_invoice' and move.x_state_dgt not in ('2', 'FI'):
            document_type_dest = 'NC'
        elif move.type == 'out_refund' and move.x_state_dgt not in ('2', 'FI'):
            document_type_dest = 'ND'
        
        data =  {
            'ref': _('Reversal of: ' + move.name),
            'date': reverse_date,
...
class FaeAccountInvoice(models.Model):
    ...
    #
    move_type = fields.Char(compute='_compute_move_type', readonly=True)  # move_type es un campo de o14 que reemplaza el campo type o13
    posted_before = fields.Boolean(compute='_compute_x_editable_generated_dgt', readonly=True)  # move_type es un campo de o14 que reemplaza el campo type o13
    ...
    def _compute_move_type(self):
        for rec in self:
            rec.move_type = rec.type   # Para facilitar usar el código de o14
    ...
    # en Odoo14 viene el método: _get_user_fiscal_lock_date
    def company_id_get_user_fiscal_lock_date(self):
        """Get the fiscal lock date for this company depending on the user"""
        self.ensure_one()
        lock_date = max(self.company_id.period_lock_date or datetime.date.min, self.company_id.fiscalyear_lock_date or datetime.date.min)
        return lock_date
    ...
    def _compute_x_editable_generated_dgt(self):
        for rec in self:
            rec.posted_before = True if rec.x_sequence else False
            ...
            lock_date = rec.company_id_get_user_fiscal_lock_date()
    ...
    def _compute_x_show_reset_to_draft_button(self):
        # reescrito para odoo13
        for inv in self:
            if self.is_invoice():
                inv.x_show_reset_to_draft_button = self.state != 'draft' and not inv.x_accounting_lock
            else:
                inv.x_show_reset_to_draft_button = self.state != 'draft' and not (inv.restrict_mode_hash_table or inv.state not in ('posted', 'cancel'))
    ...
    def _document_type_changed(self):
        ...
        elif self.type == 'out_refund' and self.x_document_type != 'NC':
            self.x_document_type = 'NC'
        elif self.type == 'out_invoice' and self.x_document_type not in ('FE','TE','FEE','ND'):
        ...
        elif self.type.find('out_') == 0 and self.x_document_type == 'FEC': 
    ...
    # override para parecerlo a o14 que valida posted_before
    def unlink(self):
        for move in self:
            ...
            if not inv.is_invoice() or inv.x_state_dgt:
                ...
                if inv.company_id.x_fae_mode in ('api-stag', 'api-prod'):
                    if (inv.type in ('out_invoice','out_refund')
                        or inv.type == 'in_invoice' and inv.x_document_type == 'FEC'):
                        gen_doc_electronico = True
                
                if not gen_doc_electronico:
                    if inv.type in ('in_invoice','in_refund') and inv.x_fae_incoming_doc_id:
                    ...
                    if inv.is_sale_document():
                        ...
                        if inv.type == 'out_invoice' and not inv.x_payment_method_id:
                            raise UserError('Debe indicar el método de pago')
                        if inv.type == 'out_refund' and not inv.x_reference_code_id:
                    ...
                    if not (inv.x_electronic_code50 or inv.x_sequence):
                        if inv.type == 'out_invoice': # Factura a Cliente
                        ...
                        elif inv.type == 'out_refund':  # Notas de Crédito
            ...
            try:
                ...
                if write_log:
                    _logger.info('>> generate_xml_and_send:  - fae_mode: %s,  identif_type: %s, move_type: %s,  sequence: %s'
                                , inv.company_id.x_fae_mode, inv.company_id.x_identification_type_id.code, inv.type, inv.x_sequence )
                ...
                if not inv.x_xml_comprobante or (inv.x_document_type == 'FEC' and inv.x_state_dgt == '2'):
                    ...
                    if inv.type == 'out_refund' and inv.x_document_type != 'NC':  # Notas de Crédito
                        inv.x_document_type = 'NC'
                    ...
                    for inv_line in inv.invoice_line_ids:
                        ...
                        if inv_line.display_type in ('line_note','line_section') or inv_line.exclude_from_invoice_tab:
                            continue
    ...
    def action_invoice_sent(self):
        ...
        if email_template and email_template.lang:
            lang = email_template._render_template(email_template.lang, 'account.move', self.id)
        if not lang:
